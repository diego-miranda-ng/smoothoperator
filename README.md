...
func main() {
    ...
    op.AddHandler("my-worker", myHandler{}, smoothoperator.Config{})
    op.StartAll()
...
- **`NewOperator(ctx)`** creates an operator. Use `AddHandler(name, handler, config)`, then `Start(name)` or `StartAll()`, and `Stop(name)` / `StopAll()` for shutdown. Pass `Config{}` for defaults, or set `Config{MaxPanicAttempts: n}` to stop a worker after n panic recoveries.
